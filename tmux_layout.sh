...
### Created by Dennis Melamed ( DennisMelamedKitware ) on 2022-08-11
### Based on https://github.com/pforret/bashew 1.18.7
script_version="0.0.1" # if there is a VERSION.md in this script's folder, it will take priority for version number
readonly script_author="dennis.melamed@kitware.com"
readonly script_created="2022-08-11"
...
option|n|name|name of the session|my_ml_workspace
choice|1|action|action to perform|ml,general_dev,check,env,update
...
Script:main() {
  ...
  ml)
    ...
    window=0
    tmux rename-window -t $session:$window 'monitors'
    tmux send-keys -t $session:$window 'tensorboard --logdir='
    tmux split-window -t $session:$window -v -p 99
    tmux send-keys -t $session:$window 'watch -n 5 nvidia-smi' C-m
    tmux split-window -t $session:$window -h -t 0
    tmux send-keys -t $session:$window 'watch -n 5 squeue' C-m
    tmux split-window -t $session:$window -h -t 2
    tmux send-keys -t $session:$window 'htop' C-m
    window=1
    tmux new-window -t $session:$window -n 'vim'
    tmux split-window -t $session:$window
    tmux send-keys -t $session:$window 'ipython' C-m
...
Option:usage() {
  ...
  IO:print "Purpose : ${txtInfo}script to generate a tmux layout with useful windows/panes for ml development${txtReset}"
